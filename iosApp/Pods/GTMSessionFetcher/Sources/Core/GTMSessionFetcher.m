...
NS_ASSUME_NONNULL_BEGIN
        ...
        NSString
*
const kGTMSessionFetcherStartedNotification = @"kGTMSessionFetcherStartedNotification";
...
- (void)logNowWithError:(nullable NSError
...
static NSDictionary *_Nullable
GTMErrorUserInfoForData(NSData
*
_Nullable data,
        NSDictionary
*
_Nullable responseHeaders
) {
NSMutableDictionary *userInfo = [NSMutableDictionary dictionary];

if (data.length > 0) {
userInfo[kGTMSessionFetcherStatusDataKey] =
data;

NSString *contentType = responseHeaders[@"Content-Type"];
if (contentType) {
userInfo[kGTMSessionFetcherStatusDataContentTypeKey] =
contentType;
}
}

return userInfo.count > 0 ? userInfo :
nil;
}

static GTMSessionFetcherTestBlock _Nullable
gGlobalTestBlock;
...
+ (instancetype)fetcherWithRequest:(nullable NSURLRequest
...
+ (instancetype)fetcherWithURLString:(NSString *)requestURLString {
    return [self fetcherWithURL:(NSURL * )
    [NSURL URLWithString:requestURLString]];
}
...
- (instancetype)initWithRequest:(nullable NSURLRequest
...
- (void)beginFetchMayDelay:(BOOL)mayDelay
    ...
    GTMSESSION_LOG_DEBUG_VERBOSE(
            @"%@ %p URL:%@ beginFetchMayDelay:%d mayAuthorize:%d mayDecorate:%d",
    ...
    if (self.sessionTask != nil) {
        ...
        GTMSESSION_ASSERT_DEBUG(NO, @"Fetch object %@ being reused; this should never happen",
                                self);
    ...
    if (shouldCheckSecurity) {
        ...
        BOOL isSecure =
                requestScheme != nil &&
                [requestScheme caseInsensitiveCompare:@"https"] == NSOrderedSame;
        if (!isSecure) {
...
#endif
                [self failToBeginFetchWithError:beginFailureError(
                        GTMSessionFetcherErrorInsecureRequest)];
                return;
            }
        }  // !isSecure
    ...
    self.canShareSession =
            (_service != nil) && !isRecreatingSession && !self.usingBackgroundSession;

    if (!self.session) {
        if (self.canShareSession) {
            self.session = [_service
                    sessionWithCreationBlock:^NSURLSession *(
    ...
    NSString *
    effectiveHTTPMethod = [fetchRequest valueForHTTPHeaderField:@"X-HTTP-Method-Override"];
    ...
    if (_downloadResumeData) {
        ...
        if (bodyFileURL) {
            ...
            GTMSESSION_ASSERT_DEBUG_OR_LOG(
                    newSessionTask, @"Failed uploadTaskWithRequest for %@, %@, body data %lu bytes",
                    _session,
    ...
    if (!newSessionTask) {
        ...
        [self failToBeginFetchWithError:beginFailureError(
                GTMSessionFetcherErrorTaskCreationFailed)];
        return;
    }
...
- (NSURLSession *)createSessionWithDelegate:(id <NSURLSessionDelegate>)sessionDelegate
                          sessionIdentifier:(nullable NSString

*)priorSessionIdentifier
...
- (void)updateUserAgentAsynchronouslyForRequest:(NSMutableURLRequest *)fetchRequest
                              userAgentProvider:(id <GTMUserAgentProvider>)userAgentProvider
                                   ...
                                   mayAuthorize:(BOOL)mayAuthorize
                                    mayDecorate:(BOOL)mayDecorate {
    GTMSESSION_LOG_DEBUG_VERBOSE(
            @"GTMSessionFetcher fetching User-Agent from GTMUserAgentProvider %@...",
            _userAgentProvider);
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (!strongSelf) {
            GTMSESSION_LOG_DEBUG_VERBOSE(
                    @"GTMSessionFetcher deallocated before GTMUserAgentProvider "
    ...
    _testBlock(self, ^(NSURLResponse *response, NSData *responseData, NSError *error) {
        ...
        if (bodyStreamProvider) {
            bodyStreamProvider(^(NSInputStream *bodyStream) {
                ...
                [self
                        invokeOnCallbackQueue:dispatch_get_global_queue(
                                DISPATCH_QUEUE_PRIORITY_HIGH, 0)
                             afterUserStopped:NO
                                        block:^{
                                            NSError * streamError;
                                            NSData * streamedData =
                                                    GTMDataFromInputStream(bodyStream,
                                                                           &streamError);
...
- (void)simulateDataCallbacksForTestBlockWithBodyData:(nullable NSData
    ...
    @synchronized (self) {
        ...
        if (response && didReceiveResponseBlock) {
            __auto_type block = ^{
                didReceiveResponseBlock(response,
                                        ^(NSURLSessionResponseDisposition desiredDisposition) {
                                            // For simulation, we'll assume
        ...
        if (destinationFileURL) {
            ...
            if ((accumulateDataBlock || receivedProgressBlock) && responseData) {
                __auto_type block = ^(NSData *data, int64_t bytesReceived,
            ...
            if (willCacheURLResponseBlock) {
                ...
                NSData * cachedData = responseData
                                      ?: [[NSData alloc] init];  // Always have non-nil data.
                NSCachedURLResponse *cachedResponse =
                        [[NSCachedURLResponse alloc] initWithResponse:response data:cachedData];
                __auto_type block = ^{
                    willCacheURLResponseBlock(cachedResponse,
...
- (void)simulateByteTransferWithData:(NSData *)responseData
    ...
    while (totalSent < totalDataLength) {
        ...
        [self invokeOnCallbackQueueUnlessStopped:^{
            transferBlock(chunkData, (int64_t) sendDataSize, (int64_t) totalSent,
...
*)activePersistedBackgroundSessions {
    NSUserDefaults *userDefaults = [[self class] fetcherUserDefaults];
...
- (void)setSessionIdentifier:(NSString *)sessionIdentifier {
    ...
    @synchronized (self) {
        ...
        GTMSESSION_ASSERT_DEBUG(!_session,
                                @"Unable to set session identifier after session created");
...
- (void)setSessionIdentifierInternal:(nullable NSString
...
*)sessionUserInfo {
    @synchronized (self) {
        ...
        if (_sessionUserInfo == nil) {
            ...
            NSSet *keysToRemove = [metadata keysOfEntriesPassingTest:^BOOL(id key, id obj,
                                                                           BOOL *stop) {
...
- (void)setSessionUserInfo:(nullable NSDictionary
...
*)sessionIdentifierDefaultMetadata {
...
- (void)restoreDefaultStateForSessionIdentifierMetadata {
    ...
    NSString *
    destinationFileURLString = metadata[kGTMSessionIdentifierDestinationFileURLMetadataKey];
...
*)sessionIdentifierMetadataUnsynchronized {
    GTMSessionCheckSynchronized(self);
...
- (NSString *)createSessionIdentifierWithMetadata:(nullable NSDictionary
...
// legacy reasons.
- (void)authorizer:(nullable id __unused
...
- (void)applyDecoratorsAtRequestWillStart:(NSArray

<id <GTMFetcherDecoratorProtocol>> *)
decorators
        startingAtIndex
:(NSUInteger)index {
    GTMSessionCheckNotSynchronized(self);
    if (index >= decorators.count) {
        GTMSESSION_LOG_DEBUG_VERBOSE(
                @"GTMSessionFetcher decorate requestWillStart %zu decorators complete",
                decorators.count);
    ...
    [decorator fetcherWillStart:self
              completionHandler:^(NSURLRequest *_Nullable newRequest, NSError *_Nullable error) {
                  ...
                  [strongSelf applyDecoratorsAtRequestWillStart:decorators startingAtIndex:index +
                                                                                           1];
              }];
}

- (void)applyDecoratorsAtRequestDidFinish:(NSArray
...
:(NSUInteger)
index
        shouldReleaseCallbacks
:(BOOL)shouldReleaseCallbacks {
    GTMSessionCheckNotSynchronized(self);
    if (index >= decorators.count) {
        GTMSESSION_LOG_DEBUG_VERBOSE(
                @"GTMSessionFetcher decorate requestDidFinish %zu decorators complete",
                decorators.count);
    ...
    [decorator fetcherDidFinish:self
              ...
              completionHandler:^{
                  GTMSESSION_LOG_DEBUG_VERBOSE(
                          @"GTMSessionFetcher decorator requestDidFinish index %zu complete",
                          index);
                  __strong __typeof__(self) strongSelf = weakSelf;
                  if (!strongSelf) {
...
#pragma clang diagnostic ignored "-Wunused"
    ...
    // service.
    dispatch_queue_t
    NS_VALID_UNTIL_END_OF_SCOPE holdCallbackQueue;
    GTMSessionFetcherCompletionHandler NS_VALID_UNTIL_END_OF_SCOPE
    holdCompletionHandler;
#pragma clang diagnostic pop
    @synchronized (self) {
        GTMSessionMonitorSynchronized(self);
        ...
        if (_sessionTask) {
            ...
            if ([oldTask state] != NSURLSessionTaskStateCompleted) {
                ...
                if (!mayResume) {
                    ...
                    [(NSURLSessionDownloadTask *) oldTask cancelByProducingResumeData:^(
                            NSData *resumeData) {
...
*_Nullable))handler {
    ...
    GTMSESSION_LOG_DEBUG_VERBOSE(
            @"%@ %p URLSession:%@ task:%@ willPerformHTTPRedirection:%@ newRequest:%@",
            [self class],
    ...
    if (redirectRequest && redirectResponse) {
        ...
        if (willRedirectBlock) {
            @synchronized (self) {
                GTMSessionMonitorSynchronized(self);
                __auto_type block = ^{
                    willRedirectBlock(redirectResponse, redirectRequest,
 ...
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))handler {
    ...
    @synchronized (self) {
        ...
        if (receivedResponseBlock) {
            ...
            __auto_type block = ^{
                receivedResponseBlock(response,
                                      ^(NSURLSessionResponseDisposition desiredDisposition) {
...
didReceiveChallenge:(NSURLAuthenticationChallenge *)challenge
  completionHandler:(void (^)(NSURLSessionAuthChallengeDisposition disposition,
    ...
    [self setSessionTask:task];
    GTMSESSION_LOG_DEBUG_VERBOSE(@"%@ %p URLSession:%@ task:%@ didReceiveChallenge:%@",
                                 [self class],
...
*)redirectURLWithOriginalRequestURL:(
nullable NSURL
*)
originalRequestURL
        redirectRequestURL
...
*)redirectRequestURL {
    ...
    if (!insecureToSecureRedirect &&
        ...
        [NSURLComponents componentsWithURL:(NSURL * _Nonnull)
        redirectRequestURL
            ...
            if (!afterStopped && !self->_stopFetchingTriggersCompletionHandler) {
                ...
                @synchronized (self) {
                    ...
                    if (serviceStoppedAllDate &&
                        [self->_initialBeginFetchDate compare:serviceStoppedAllDate] !=
                        NSOrderedDescending) {
...
- (void)invokeFetchCallbacksOnCallbackQueueWithData:(nullable NSData
...
didFinishDownloadingToURL:(NSURL *)downloadLocationURL {
    ...
    [self setSessionTask:downloadTask];
    GTMSESSION_LOG_DEBUG_VERBOSE(
            @"%@ %p URLSession:%@ downloadTask:%@ didFinishDownloadingToURL:%@",
        ...
        if (statusCode < 200 || statusCode > 399) {
            ...
            if (_downloadedLength > 0 && _downloadedLength <= kMaximumDownloadErrorDataLength) {
                ...
                GTMSESSION_LOG_DEBUG(
                        @"Download error data for file %@ not passed to userInfo due to size "
                        @"%lld",
...
didCompleteWithError:(NSError *)error {
    [self setSessionTask:task];
    GTMSESSION_LOG_DEBUG_VERBOSE(@"%@ %p URLSession:%@ task:%@ didCompleteWithError:%@",
                                 [self class],
...
#endif
        userStoppedTriggerCompletion =
                _userStoppedFetching && _stopFetchingTriggersCompletionHandler;
    ...
    if ((status == 403) && self.usingBackgroundSession) {
        ...
        if (![request.URL isEqual:redirectURL]) {
            NSString *
            authorizationHeader = [request.allHTTPHeaderFields objectForKey:@"Authorization"];
...
- (void)URLSession:(NSURLSession *)session didBecomeInvalidWithError:(nullable NSError
...
- (void)finishWithError:(nullable NSError
...
- (void)shouldRetryNowForStatus:(NSInteger)status
    ...
    @synchronized (self) {
        ...
        if (canRetry) {
            ...
            willRetry =
                    shouldRetryForAuthRefresh || forceAssumeRetry || [self isRetryError:error] ||
...
- (NSTimeInterval)retryAfterInterval {
    ...
    NSTimeInterval retryAfterInterval =
            (retryAfterDate != nil) ? retryAfterDate.timeIntervalSinceNow
                                    : retryAfterValue.intValue;
...
- (void)setRequest:(nullable NSURLRequest
...
- (void)updateMutableRequest:(nullable NSMutableURLRequest
...
- (void)setRequestValue:(nullable NSString
...
- (void)updateRequestValue:(nullable NSString
...
- (void)setResponse:(nullable NSURLResponse
...
- (void)setBodyFileURL:(nullable NSURL
...
<GTMFetcherAuthorizationProtocol>)authorizer {
    @synchronized (self) {
        GTMSessionMonitorSynchronized(self);
...
- (void)setAuthorizer:(nullable id
...
- (void)setDownloadedData:(nullable NSData
...
- (void)setSession:(nullable NSURLSession
...
- (void)setSessionNeedingInvalidation:(nullable NSURLSession
...
*)queue {
    @synchronized (self) {
        ...
        if (queue != _delegateQueue) {
            if ([self isFetchingUnsynchronized]) {
                GTMSESSION_ASSERT_DEBUG(0,
                                        @"sessionDelegateQueue should not change after fetch begins");
            } else {
...
- (void)setDestinationFileURL:(nullable NSURL
...
- (void)setProperties:(nullable NSDictionary
...
- (void)setCookies:(nullable NSArray
...
<NSHTTPCookie *> *))completionHandler {
...
//
// Note: this should only be called from inside a @synchronized(self) block
- (nullable NSHTTPCookie

*)cookieMatchingCookie:(NSHTTPCookie *)cookie {
    GTMSessionCheckSynchronized(self);
    ...
    GTMSESSION_ASSERT_DEBUG(name && domain && path,
                            @"Invalid stored cookie (name:%@ domain:%@ path:%@)", name, domain,
                            path);
...
- (instancetype)initWithBundle:(nullable NSBundle
...
NSString *GTMFetcherStandardUserAgentString(NSBundle *_Nullable bundle) {
    NSString * result = [NSString stringWithFormat:@"%@ %@", GTMFetcherApplicationIdentifier(
            bundle),
