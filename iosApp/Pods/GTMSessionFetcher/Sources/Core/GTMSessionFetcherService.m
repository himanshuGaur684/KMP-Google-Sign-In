...
// clang-format off
@synthesize maxRunningFetchersPerHost = _maxRunningFetchersPerHost, configuration = _configuration, configurationBlock = _configurationBlock, cookieStorage = _cookieStorage, challengeBlock = _challengeBlock, credential = _credential, proxyCredential = _proxyCredential, allowedInsecureSchemes = _allowedInsecureSchemes, allowLocalhostRequest = _allowLocalhostRequest, allowInvalidServerCertificates = _allowInvalidServerCertificates, retryEnabled = _retryEnabled, retryBlock = _retryBlock, maxRetryInterval = _maxRetryInterval, minRetryInterval = _minRetryInterval, metricsCollectionBlock = _metricsCollectionBlock, properties = _properties, unusedSessionTimeout = _unusedSessionTimeout, userAgentProvider = _userAgentProvider, decoratorsPointerArray = _decoratorsPointerArray, testBlock = _testBlock, stopFetchingTriggersCompletionHandler = _stopFetchingTriggersCompletionHandler;
// clang-format on
...
- (nonnull dispatch_queue_t)serialQueueForNewFetcher:(GTMSessionFetcher *)fetcher {
    @synchronized (self) {
        ...
        return dispatch_queue_create_with_target(kQueueLabel, DISPATCH_QUEUE_SERIAL,
                                                 _callbackQueue);
    }
}
...
<id <GTMFetcherDecoratorProtocol>> *)decorators {
    @synchronized (self) {
...
- (void)fetcherDidBeginFetching:(GTMSessionFetcher *)fetcher {
    ...
    if (delegateDispatcher) {
        GTMSESSION_ASSERT_DEBUG(fetcher.canShareSession, @"Inappropriate shared session: %@",
                                fetcher);
        ...
        GTMSESSION_ASSERT_DEBUG(fetcherSession == sharedSession,
                                @"Inconsistent session: %@ %@ (shared: %@)", fetcher,
                                fetcherSession,
                                sharedSession);
...
- (NSArray *)issuedFetchersWithRequestURL:(NSURL *)requestURL {
    ...
    NSIndexSet *indexes = [allFetchers
            indexesOfObjectsPassingTest:^BOOL(GTMSessionFetcher *fetcher, NSUInteger idx,
                                              BOOL *stop) {
...
- (void)setUserAgent:(nullable NSString
...
<GTMUserAgentProvider>)userAgentProvider {
    @synchronized (self) {
        return _userAgentProvider;
    }
}

- (void)setUserAgentProvider:(nullable id
...
+ (instancetype)mockFetcherServiceWithFakedData:(NSData *)fakedDataOrNil
    ...
    NSHTTPURLResponse *fakedResponse =
            [[NSHTTPURLResponse alloc] initWithURL:url
                                        statusCode:(fakedErrorOrNil ? 500
...
- (NSString *)description {
    return
            [NSString stringWithFormat:@"%@ %p %@ %@", [self class], self, _session
                                                                           ?: @"<no session>",
