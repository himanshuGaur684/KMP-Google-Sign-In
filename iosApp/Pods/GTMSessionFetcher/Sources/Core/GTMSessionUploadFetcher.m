...
int64_t const kGTMSessionUploadFetcherStandardChunkSize = (int64_t)
LLONG_MAX;
...
+ (instancetype)uploadFetcherForSessionIdentifierMetadata:(NSDictionary *)metadata {
    ...
    NSString *
    uploadLocationURLString = metadata[kGTMSessionIdentifierUploadLocationURLMetadataKey];
...
- (void)setLocationURL:(nullable NSURL
        ...
        if (_uploadData) {
            ...
            if (_uploadFileLength == kGTMSessionUploadFetcherUnknownFileSize) {
                if (_uploadFileHandle) {
                    ...
                    _uploadFileLength = (int64_t)
                    [_uploadFileHandle seekToEndOfFile];
                } else if (_uploadDataProvider) {
                    // _uploadFileLength is set when the _uploadDataProvider is set.
                    GTMSESSION_ASSERT_DEBUG(_uploadFileLength >= 0,
                                            @"No uploadDataProvider length set");
                } else {
...
- (void)setChunkFetcher:(nullable GTMSessionFetcher
...
- (void)setFetcherInFlight:(nullable GTMSessionFetcher
...
- (void)beginChunkFetches {
    ...
    NSString *
    uploadLocationURLStr = [responseHeaders objectForKey:kGTMSessionHeaderXGoogUploadURL];
...
- (void)sendQueryForUploadOffsetWithFetcherProperties:(NSDictionary *)props {
    ...
    [queryFetcher
            setCommentWithFormat:@"%@ (query offset)", originalComment ? originalComment
                                                                       : @"upload"];
        ...
        [self generateChunkSubdataWithOffset:offset
                                    ...
                                    response:^(NSData *chunkData, int64_t uploadFileLength,
                                        ...
                                        dispatch_async(dispatch_get_main_queue(), ^{
                                            ...
                                            // upload starts.
                                            if (self.uploadDataProvider != nil &&
                                                uploadFileLength > 0) {
...
- (GTMSessionFetcher *)uploadFetcherWithProperties:(NSDictionary *)properties
    ...
    if ([self isRetryEnabled]) {
        ...
        chunkFetcher.retryBlock =
                ^(BOOL suggestedWillRetry, NSError *chunkError,
...
- (void)chunkFetcher:(GTMSessionFetcher *)chunkFetcher
    ...
    if (error || (needsQuery && !isQueryFetch)) {
        ...
        if (self.shouldInitiateOffsetQuery || (needsQuery && !isQueryFetch) ||
            ([error.domain isEqual:kGTMSessionFetcherStatusDomain] && status >= 400 &&
             status <= 499 &&
...
// Public properties.
// clang-format off
@synthesize currentOffset = _currentOffset, allowsCellularAccess = _allowsCellularAccess, delegateCompletionHandler = _delegateCompletionHandler, chunkFetcher = _chunkFetcher, lastChunkRequest = _lastChunkRequest, subdataGenerating = _subdataGenerating, shouldInitiateOffsetQuery = _shouldInitiateOffsetQuery, uploadGranularity = _uploadGranularity, uploadRetryFactor = _uploadRetryFactor;
// clang-format on
