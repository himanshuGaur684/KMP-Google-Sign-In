...
- (instancetype)initWithNetworkLoggerDelegate:(id <GULNetworkLoggerDelegate>)networkLoggerDelegate {
    ...
    if (self) {
        ...
        NSArray *tempPathComponents = @[
                storageDirectory, kGULNetworkApplicationSupportSubdirectory,
...
*)sessionIDFromAsyncPOSTRequest:(NSURLRequest *)
request
        completionHandler
:
...
*)sessionIDFromAsyncGETRequest:(NSURLRequest *)
request
        completionHandler
:(GULNetworkURLSessionCompletionHandler)handler {
...
didCompleteWithError:(NSError *)error {
    ...
    if (task.response) {
        ...
        NSAssert([task.response isKindOfClass:[NSHTTPURLResponse class]],
                 @"URL response must be HTTP");
        ...
        error = [[NSError alloc]
                initWithDomain:kGULNetworkErrorDomain
                      ...
                      userInfo:@{
                              kGULNetworkErrorContext: @"Network Error: Empty network response"}];
    }
                ...
                [self->_loggerDelegate
                        GULNetwork_logWithLevel:kGULNetworkLogLevelError
                                    messageCode:kGULNetworkMessageCodeURLSession008
                                       ...
                                       contexts:@[@((int) CFErrorGetCode(errorRef)),
                                                  self->_request.URL]];
                CFRelease(errorRef);
            }
...
<NSString *, GULNetworkURLSession *> *)sessionIDToFetcherMap {
...
+ (nullable GULNetworkURLSession

*)sessionFromFetcherMapForSessionID:(NSString *)sessionID {
    [[self sessionIDToFetcherMapReadWriteLock] lock];
