...
- (void)getTokenWithLimitedUse:(BOOL)limitedUse
                    completion:(void (^)(GACAppCheckToken *_Nullable, NSError
        ...
        if (self.ongoingGetTokenOperation) {
            ...
            if (limitedUse || self.ongoingGetTokenOperationLimitedUse != limitedUse) {
                return self.ongoingGetTokenOperation.thenOn(
                        self.queue, ^id
                _Nullable(GACAppCheckToken * _Nullable
                value) {
                                     ...
                                     self.queue, ^id(GACAppAttestProviderState *attestState) {
                                         switch (attestState.state) {
                                             case GACAppAttestAttestationStateUnsupported:
                                                 GACAppCheckLogDebug(
                                                         GACLoggerAppCheckMessageCodeAppAttestNotSupported,
...
- (FBLPromise<GACAppCheckToken *> *)initialHandshakeWithKeyID:(nullable NSString
...
:(BOOL)limitedUse {
    ...
    return [self attestKeyGenerateIfNeededWithID:keyID limitedUse:limitedUse].thenOn(
            self.queue,
            ^FBLPromise<GACAppCheckToken *> *(
                    NSArray * /*[keyID, attestArtifact]*/ attestationResults) {
...
- (FBLPromise<GACAppAttestKeyAttestationResult *> *)attestKey:(NSString *)keyID
    ...
    .thenOn(self.queue,
            ...
            .thenOn(self.queue,
                    ^FBLPromise<GACAppAttestKeyAttestationResult *> *(NSData *attestation) {
...
attestKeyGenerateIfNeededWithID:(nullable NSString
            ...
            .recoverOn(self.queue,
                       ^id(NSError *error) {
                           ...
                           if (underlyingError &&
                               ...
                               return [self resetAttestation].thenOn(self.queue,
                                                                     ^NSError *(id result) {
...
- (FBLPromise<NSNull *> *)isAppAttestSupported {
    if (self.appAttestService.isSupported) {
        ...
        NSError *
        error = [GACAppCheckErrorUtil unsupportedAttestationProvider:@"AppAttestProvider"];
...
- (FBLPromise<NSString *> *)generateAppAttestKeyIDIfNeeded:(nullable NSString
