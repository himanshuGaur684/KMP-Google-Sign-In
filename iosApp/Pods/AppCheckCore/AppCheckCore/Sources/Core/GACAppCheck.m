...
kTokenExpirationThreshold = 5 * 60;  // 5 min.
...
<GACAppCheckTokenDelegate>)tokenDelegate {
    self = [super init];
    if (self) {
        ...
        tokenRefresher.tokenRefreshHandler = ^(GACAppCheckTokenRefreshCompletion _Nonnull
                                               completion) {
...
- (void)tokenForcingRefresh:(BOOL)forcingRefresh completion:(GACAppCheckTokenHandler)handler {
    [self retrieveOrRefreshTokenForcingRefresh:forcingRefresh]
            .then(^id
    _Nullable(GACAppCheckToken * token)
    {
